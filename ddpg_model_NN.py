...
path = 'Results/'+environment+ '_seed='+str(seed)+'_run='+str(run)+'_episodes='+str(total_epi)+ '_ddpg_episode_reward_modelbased.npy'
pathmodel = 'Results/'+environment+ '_seed='+str(seed)+'_run='+str(run)+'_episodes='+str(total_epi)+ '_ddpg_modelbased'
...
with tf.Session() as sess:
    ...
    while episode_number <= total_epi:
        ...
        x = np.array(x)
        action = y = env.action_space.sample()
        ...
        if done_steps == 100: 
            ...
            if trainTheModel == True:
                ...
                tr_loss_part,_,_ = sess.run([logits,output_pred,optimizer],feed_dict={features:state_prevs, states : state_nexts, true_reward: rewards})
                print('Episode number=',episode_number,', Training error = ',tr_loss_part)

            if trainThePolicy == True:
                model = DDPG.load(pathmodel)
                n = model.learn(total_timesteps=model_train_steps,seed = seed,path =path,environment = environment,tf_sess = sess)
...
class DDPG(OffPolicyRLModel):
    ...
    def learn(self, total_timesteps,path,environment,tf_sess, callback=None, seed=None, log_interval=100, tb_log_name="DDPG",
                    ...
                    for _ in range(log_interval):
                        ...
                        for _ in range(self.nb_rollout_steps):
                            ...
                            if rank == 0 and self.render:
                                self.env.render()
                            done = False    #Swimmer specific
                            s_a_input = [[new_obs] + [action]]
                            new_obs, reward = tf_sess.run([output_pred,pred_rwd],feed_dict={features:s_a_input})
                            ...
                            if done:
                                ...
                                np.save(path,epoch_episode_rewards)

                                ################################# Added reward after each episode.
                                # reward_keep.append(episode_reward)
                                ##########################################################
                        ...
                        if self.eval_env is not None:
                            ...
                            for _ in range(self.nb_eval_steps):
                                ...
                                eval_done = False       #Swimmer specific

                                s_a_input = [[eval_obs] + [eval_action]]
                                eval_obs, eval_r = tf_sess.run([output_pred,pred_rwd],feed_dict={features:s_a_input})
