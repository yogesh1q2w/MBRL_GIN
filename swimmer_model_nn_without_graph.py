...
def build_model(input, num_dense_layers,list_nodes):
	...
	b6 = tf.Variable(tf.zeros([]), name="Bias_end", dtype=tf.float32)
...
def plotter(Y_pred,Y_truth,type):
	...
	stateinf = ['x','y','gamma','theta1','theta2','der_gamma','der_theta_1','der_theta_2']
	for i in range(len(Y_pred[0])):
		fig, ax = plt.subplots(figsize=(7,3))
		agent_name=str('State information estimated = '+stateinf[i]+', Type = ' + type)
		# plt.subplot(12, 2, 2*i + j)
		ax.plot(Y_pred[:,i],label='Model based,without graph structure', linewidth=2, color='g')
		# plt.subplot(12, 2, 2*i + j)
		...
		fig.savefig(agent_name + '_comparison_without_graph.png', dpi=200, bbox_inches='tight')
		plt.show()
...
def train_network(list_nodes,train_data,train_label,test_data,test_label,valid_data,valid_label,dimension,learning_rate=0.0001,epochs=300,mini_batch_num=256):	
	...
	output_pred =  build_model(features,num_layers-2, list_nodes)
	logits = tf.reduce_mean(tf.square(output_pred - states))
	...
	if train:
		...
		for i in range(epochs):
			...
			for j in range(int(len(train_data) / mini_batch_num)):
				...
				tr_loss_part,_,_ = sess.run([logits,output_pred,optimizer],feed_dict={features:batch_data, states : batch_label})
			...
			for j in range(int(len(valid_data) / mini_batch_num)):
				...
				# else:
				val_loss_part,_,_ = sess.run([logits,output_pred,optimizer],feed_dict={features:batch_data, states : batch_label})
	...
	if test:
		...
		if one_step:
			...
			test_loss_part,estimated = sess.run([logits,output_pred],feed_dict={features:batch_data, states : batch_label})
...
def load_data():
	...
	return scaler,norm_train_data,train_output,norm_test_data,test_output,norm_valid_data,val_output


scaler,train_data,train_label,test_data,test_label,valid_data,valid_label = load_data()
train_network([128,128],train_data,train_label,test_data,test_label,valid_data,valid_label,dimension=10)