...
from stable_baselines.trpo_mpi.utils import traj_segment_generator_env, traj_segment_generator_model, add_vtarg_and_adv, flatten_lists
...
class PPO1(ActorCriticRLModel):
    ...
    def learn_env(self, total_timesteps, path, callback=None, seed=None, log_interval=100, tb_log_name="PPO1",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            with self.sess.as_default():
                ...
                seg_gen = traj_segment_generator_env(self.policy_pi, self.env, self.timesteps_per_actorbatch)
                ...
                while True:
                    ...
                    if self.verbose >= 1 and MPI.COMM_WORLD.Get_rank() == 0:
                        logger.dump_tabular()
        # reward_epi = seg['true_rew']
        # np.save(path,np.array(reward_epi))

        return samples


    def learn_model(self, model,total_timesteps, path, callback=None, seed=None, log_interval=100, tb_log_name="PPO1",
              reset_num_timesteps=True):
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            with self.sess.as_default():
                ...
                # Prepare for rollouts
                seg_gen = traj_segment_generator_model(model, self.policy_pi, self.env, self.timesteps_per_actorbatch)
