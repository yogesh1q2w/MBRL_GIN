...
def load_data():
	train_features = np.load('Data/train_input_s_a.npy')
	train_output = np.load('Data/train_output_s.npy')

	test_features = np.load('Data/test_input_s_a.npy')
	test_output = np.load('Data/test_output_s.npy')

	val_features = np.load('Data/val_input_s_a.npy')
	val_output = np.load('Data/val_output_s.npy')

	set_num = 1000

	train_features = train_features[:set_num] 
	train_output = train_output[:set_num]

	test_features = test_features[:set_num]
	test_output = test_output[:set_num]

	val_features = val_features[:set_num]
	val_output = val_output[:set_num]

	state_train = np.array([[[train_features[:,0],train_features[:,5],train_features[:,3],train_features[:,4]],
							[train_features[:,1],train_features[:,6],train_features[:,3],train_features[:,4]],
							[train_features[:,2],train_features[:,7],train_features[:,3],train_features[:,4]]],
							[[train_output[:,0],train_output[:,5],train_output[:,3],train_output[:,4]],
							[train_output[:,1],train_output[:,6],train_output[:,3],train_output[:,4]],
							[train_output[:,2],train_output[:,7],train_output[:,3],train_output[:,4]]]])

	state_train = np.rollaxis(state_train,3)
	state_train = np.rollaxis(state_train,3,2)


	action_train = np.array([np.zeros(set_num),train_features[:,8],train_features[:,9]])
	action_train = np.rollaxis(action_train,1)
	action_train = np.reshape(action_train,(set_num,1,3))


	state_test = np.array([[[test_features[:,0],test_features[:,5],test_features[:,3],test_features[:,4]],
							[test_features[:,1],test_features[:,6],test_features[:,3],test_features[:,4]],
							[test_features[:,2],test_features[:,7],test_features[:,3],test_features[:,4]]],
							[[test_output[:,0],test_output[:,5],test_output[:,3],test_output[:,4]],
							[test_output[:,1],test_output[:,6],test_output[:,3],test_output[:,4]],
							[test_output[:,2],test_output[:,7],test_output[:,3],test_output[:,4]]]])

	state_test = np.rollaxis(state_test,3)
	state_test = np.rollaxis(state_test,3,2)

	action_test = np.array([np.zeros(set_num),test_features[:,8],test_features[:,9]])
	action_test = np.rollaxis(action_test,1)
	action_test = np.reshape(action_test,(set_num,1,3))


	state_val = np.array([[[val_features[:,0],val_features[:,5],val_features[:,3],val_features[:,4]],
							[val_features[:,1],val_features[:,6],val_features[:,3],val_features[:,4]],
							[val_features[:,2],val_features[:,7],val_features[:,3],val_features[:,4]]],
							[[val_output[:,0],val_output[:,5],val_output[:,3],val_output[:,4]],
							[val_output[:,1],val_output[:,6],val_output[:,3],val_output[:,4]],
							[val_output[:,2],val_output[:,7],val_output[:,3],val_output[:,4]]]])

	state_val = np.rollaxis(state_val,3)
	state_val = np.rollaxis(state_val,3,2)

	action_val = np.array([np.zeros(set_num),val_features[:,8],val_features[:,9]])
	action_val = np.rollaxis(action_val,1)
	action_val = np.reshape(action_val,(set_num,1,3))
...
def plotter(Y_pred,Y_truth,type):
	...
	indices = [(0,0),(0,1),(0,2),(2,0),(3,0),(1,0),(1,1),(1,2)]
	for i in range(len(stateinf)):
		fig, ax = plt.subplots(figsize=(7,3))
		agent_name=str('State information estimated = '+stateinf[i]+', Type = ' + type)
		# plt.subplot(12, 2, 2*i + j)
		ax.plot(Y_pred[:,indices[i][0],indices[i][1]],label='IN preediction', linewidth=2, color='g')
		# plt.subplot(12, 2, 2*i + j)
		ax.plot(Y_truth[:,indices[i][0],indices[i][1]],label='Ground Truth', linewidth=2, color='b')
		...
		fig.savefig('Results/'+agent_name + '_comparison_IN.png', dpi=200, bbox_inches='tight')
		plt.show()
...
# num_dense_nodes_hox,
	...
	if train:
		...
		SAVE_PATH='./models/Swimmer-v2_IN'
	...
	if test:
		...
		if recursive:
					...
					plotter(output_pred,output_true,'rec')
