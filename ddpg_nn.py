...
sys.path.append('/home/rahul/Acads/Rohan/RL_project/codes/Swimmer')
import Model
...
class DDPG(OffPolicyRLModel):
    ...
    def learn_env(self, total_timesteps, environment, callback=None, seed=None, log_interval=100, tb_log_name="DDPG",
    ...
    def learn_model(self, model, total_timesteps,path,environment, callback=None, seed=None, log_interval=100, tb_log_name="DDPG",
              reset_num_timesteps=True):
        #########################################################
        # reward_keep = []
        ######################################################
                ...
                while True:
                    for _ in range(log_interval):
                        ...
                        for _ in range(self.nb_rollout_steps):
                            ...
                            if rank == 0 and self.render:
                                self.env.render()
                            new_obs, reward, done = model.model_step(obs,action)
